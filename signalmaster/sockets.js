...
module.exports = function (server, config) {
   ...
   //config.codecPriority || ["VP9", "H264", "VP9"]config.codecPriority ||
        //var priority =  [ "VP9", "VP8","H264"];  // ordered priority list, first = highest priority
        //When Need To Modifiy Priority
        var priority = config.codecPriority || [];
        var ids = [];
        ...
        if (ids.length > 0 && details.payload && details.payload.sdp) {
            ...
            if (m !== null && m.length === 3) {
               ...
               logger.info("Setting video codec priority. \"%s\"", m[0]);//mPrioritized)
               //When Need To Modifiy Priority
    ...
    function setOpusMaxAvgBitrate(details) {
        ...
        if (ids.length > 0 && details.payload && details.payload.sdp) {
            ...
            if (m !== null && m.length === 3) {
                ...
                var mPrioritized = m[0].replace(m[2], prioritized.join(" "));
                logger.info("Setting Audio codec priority. \"%s\"", m[0]);//mPrioritized)
                //When Need To Modifiy Priority
                // details.payload.sdp = sdp.replace(m[0], mPrioritized);
    ...
    function findVP9Id(details) {
        if (details.payload && details.payload.sdp) {
            var m = details.payload.sdp.match(/a=rtpmap\:(\d+)\sVP9\/\d+/);
            if (m.length > 0) {
                return m[1];
            }
        }
